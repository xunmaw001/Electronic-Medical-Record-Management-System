            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">患者管理</li>
                        <li class="breadcrumb-item active">编辑患者</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>出生年月</label>
                                        <input id="huanzheTime-input" name="huanzheTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>患者照片</label>
                                        <img id="huanzhePhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="huanzhePhotoupload"
                                               class="form-control-file">
                                        <input name="huanzhePhoto" id="huanzhePhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#huanzhePhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('huanzhePhotoImg').setAttribute('src',photoUrl);
                document.getElementById('huanzhePhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("huanzhe/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addhuanzhe');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                huanzheAddress: "required",
                huanzheTime: "required",
                huanzhePhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                ...
                huanzheAddress: "现住地址不能为空",
                huanzheTime: "出生年月不能为空",
                huanzhePhoto: "患者照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addhuanzhe = window.sessionStorage.getItem("addhuanzhe");
        if (addhuanzhe != null && addhuanzhe != "" && addhuanzhe != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("huanzhe/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#huanzheTime-input").val(ruleForm.huanzheTime);
