    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑护理记录</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑护理记录</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">护理记录信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="huanzheId" name="huanzheId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>护理名字</label>
                                        <input id="huliName" name="huliName" class="form-control"
                                               placeholder="护理名字">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>护理类型</label>
                                        <select id="huliTypesSelect" name="huliTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>护理时间</label>
                                        <input id="huliTime-input" name="huliTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>护理详情</label>
                                        <input id="huliContentupload" name="file" type="file">
                                        <script id="huliContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var huliContentUe = UE.getEditor('huliContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="huliContent" id="huliContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#huliContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('huliContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("huli/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addhuli');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationHulitypesSelect(){
            var huliTypesSelect = document.getElementById('huliTypesSelect');
            if(huliTypesSelect != null && huliTypesOptions != null  && huliTypesOptions.length > 0 ){
                for (var i = 0; i < huliTypesOptions.length; i++) {
                    huliTypesSelect.add(new Option(huliTypesOptions[i].indexName,huliTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(huliTypesSelect != null && huliTypesOptions != null  && huliTypesOptions.length > 0 ) {
            for (var i = 0; i < huliTypesOptions.length; i++) {
                if (huliTypesOptions[i].codeIndex == ruleForm.huliTypes) {//下拉框value对比,如果一致就赋值汉字
                        huliTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.huliContent != null && ruleForm.huliContent != 'null' && ruleForm.huliContent != '' && $("#huliContentupload").length>0) {

            var huliContentUeditor = UE.getEditor('huliContentEditor');
            huliContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.huliContent != null){
                    mes = ''+ ruleForm.huliContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                huliContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#huliContentupload").length>0) {
            var huliContentEditor = UE.getEditor('huliContentEditor');
            if (huliContentEditor.hasContents()) {
                $('#huliContent-input').attr('value', huliContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    // 获取当前详情
    function getDetails() {
        var addhuli = window.sessionStorage.getItem("addhuli");
        if (addhuli != null && addhuli != "" && addhuli != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("huli/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#huliName").val(ruleForm.huliName);
        $("#huliTime-input").val(ruleForm.huliTime);
        $("#huliContent").val(ruleForm.huliContent);
